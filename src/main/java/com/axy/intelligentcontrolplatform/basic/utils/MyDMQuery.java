...
public class MyDMQuery extends AbstractDbQuery {
    public MyDMQuery() {
    }

    public MyDMQuery(String table) {
        this.table = table;
    }
    ...
    public String tablesSql() {
        return "SELECT DISTINCT T1.TABLE_NAME,T2.COMMENTS AS TABLE_COMMENT FROM USER_TAB_COLUMNS T1 " +
                "INNER JOIN USER_TAB_COMMENTS T2 ON T1.TABLE_NAME = T2.TABLE_NAME WHERE 1=1 AND T1.TABLE_NAME in ('"+table+"')";
    }
